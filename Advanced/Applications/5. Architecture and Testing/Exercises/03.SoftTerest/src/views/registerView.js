...
import { register, updateNav } from "../services/userService.js";
...
function registerTemplate() {
  return html` <div class="container home wrapper  my-md-5 pl-md-5">
    <div class="row-form d-md-flex flex-mb-equal ">
      ...
      <form @submit=${createSubmitHandler(onRegister)} class="form-user col-md-7" action="" method="">
...
async function onRegister({ password, repeatPassword, email }) {
  if (email.length < 3 || password.length < 3 || password !== repeatPassword) {
    return;
  }
  await register({ password, repeatPassword, email });
  updateNav();
  page.redirect("/");
}
